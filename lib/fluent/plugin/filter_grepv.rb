module Fluent
  class GrepvFilter < Filter
    Fluent::Plugin.register_filter('grepv', self)

    REGEXP_MAX_NUM = 20

    (1..REGEXP_MAX_NUM).each {|i| config_param :"regexp#{i}",  :string, :default => nil }
    (1..REGEXP_MAX_NUM).each {|i| config_param :"exclude#{i}", :string, :default => nil }
    config_param :invert_match, :bool, :default => true
    ...
    def configure(conf)
      ...
      (1..REGEXP_MAX_NUM).each do |i|
        ...
        @regexps[key] = {'key': key, 'regexp', Regexp.compile(exclude)}
      end
      ...
      (1..REGEXP_MAX_NUM).each do |i|
        ...
        @excludes = {'key': key, 'regexp', Regexp.compile(exclude)}
      end
    end

    def filter(tag, time, record)
      result = invert_match? record : nil
      begin
        catch(:break_loop) do
          @regexps.each do |o|
          ...
          result = invert_match? nil : record
        end
      rescue => e
